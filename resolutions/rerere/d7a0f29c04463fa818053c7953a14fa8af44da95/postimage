...
pub(crate) async fn scan_and_connect_peripherals<'a, C: Controller + ControllerCmdSync<LeSetScanParams>>(
    ...
    loop {
        ...
        if !pending.is_empty() {
            ...
            // All peripherals are connected: wait until a session ends or wired takes over.
...
>(
    ...
    // `run_central_manager_task` just applied the awake parameters.
    let mut applied = Some(false);

    loop {
        if Some(sleeping) != applied {
            ...
            if sent {
                applied = Some(sleeping);
            }
        }

        match select(sleep_events.next_event(), latency_changes.next_message_pure()).await {
            Either::First(event) => sleeping = event.0,
            // The awake parameters embed the policy, so re-apply them.
            Either::Second(()) => {
                if !sleeping {
                    applied = None;
                }
