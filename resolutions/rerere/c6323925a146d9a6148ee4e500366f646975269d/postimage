...
impl<'a> PointingProcessor<'a> {
    ...
    async fn on_pointing_event(&mut self, event: PointingEvent) {
        ...
        let previous_device_buttons = self.device_buttons;
        let device_buttons_changed = event.buttons != previous_device_buttons;
        self.device_buttons = event.buttons;
        if let PointingMode::Drag(drag_config) = self.current_mode {
            self.drag_latched = drag_latch_after(
                self.drag_latched,
                previous_device_buttons,
                event.buttons,
...
/// Build the mouse report cursor-style motion produces.
fn cursor_report(x: i16, y: i16, cfg: &CursorConfig, buttons: u8) -> MouseReport {
    let out_x = x.saturating_mul(cfg.multiplier_x as i16);
    let out_y = y.saturating_mul(cfg.multiplier_y as i16);
    let out_x = if cfg.invert_x { -out_x } else { out_x };
    let out_y = if cfg.invert_y { -out_y } else { out_y };
    MouseReport {
        buttons,
        x: out_x.clamp(i8::MIN as i16, i8::MAX as i16) as i8,
        y: out_y.clamp(i8::MIN as i16, i8::MAX as i16) as i8,
...
/// a drag has the button down for the whole gesture and the tap that ends
/// one does not re-latch when the finger lifts.
fn drag_latch_after(latched: bool, previous: u8, current: u8, toggled_by: u8) -> bool {
    let pressed_now = current & toggled_by != 0;
    let pressed_before = previous & toggled_by != 0;
...
mod tests {
    ...
    #[test]
    fn test_drag_latch_survives_the_tap_that_took_it() {
        let toggled_by = DragConfig::default().toggled_by;
        // A tap is a press followed by a release. The latch has to outlive
        // the release, or the drag would end the moment the finger lifts.
        let pressed = drag_latch_after(false, 0, toggled_by, toggled_by);
        assert!(pressed);
        let released = drag_latch_after(pressed, toggled_by, 0, toggled_by);
        assert!(released);

        // The next tap drops it, and again only on the press.
        let tapped_again = drag_latch_after(released, 0, toggled_by, toggled_by);
        assert!(!tapped_again);
        assert!(!drag_latch_after(tapped_again, toggled_by, 0, toggled_by));
    }

    #[test]
    fn test_drag_latch_ignores_other_buttons() {
        let toggled_by = 1;
        let secondary = 1 << 1;
