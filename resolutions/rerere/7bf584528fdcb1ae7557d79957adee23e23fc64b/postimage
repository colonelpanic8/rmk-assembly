...
- The morse mode, which can be set by enabling one of these:
  - `permissive_hold`: Enables permissive hold mode. When enabled, hold action will be triggered when a key is pressed and released during tap-hold decision. This is the default mode when no mode flag is set. This option is recommended to set to true when `enable_flow_tap` is set to true.
  ...
  - `tap_unless_interrupted`: Enables tap-unless-interrupted mode, the mirror of `hold_on_other_press` and the same as ZMK's `tap-unless-interrupted` flavor. Pressing any other key while the decision is pending settles the tap-hold key as a tap immediately, so the hold is reachable only by holding to `hold_timeout` with nothing else pressed. Defaults to `false`.
  - `normal_mode`: Enables normal mode, where the decision is made only when `hold_timeout` expires. Set it explicitly to get this behavior.
