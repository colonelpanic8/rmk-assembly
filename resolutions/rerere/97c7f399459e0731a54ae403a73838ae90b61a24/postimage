        ...
        loop {
            ...
            if !self.unprocessed_events.is_empty() {
                ...
                let input = if let Some(deadline) = deadline {
                    match with_deadline(deadline, self.next_input()).await {
                        Ok(input) => input,
                        Err(_) => {
                ...
                match input {
                    Input::Key(event) => self.process_inner(event).await,
                    Input::Virtual(event) => self.process_virtual_key(event).await,
                }
            };
...
impl<'a> Keyboard<'a> {
    ...
    /// Type zero-padded hex, four digits per UTF-16 unit or up to six per codepoint.
    ...
    /// OS input sequences use literal reports without changing logical held-key state.
